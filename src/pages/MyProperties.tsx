...
import {
  Building, Edit, Eye, MapPin, Bed, Bath, Home,
  DollarSign, Plus, Calendar, Search, ArrowLeft,
        ...
        <div className="px-4 py-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button variant="outline" size="sm" onClick={() => navigate('/agent-dashboard')} className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              ...
              { label: 'Draft Properties', value: properties.filter(p => p.status === 'draft').length },
              { label: 'Sold/Rented', value: properties.filter(p => p.status === 'sold' || p.status === 'rented').length },
            ].map(stat => (
              <Card key={stat.label} className="bg-white/20 backdrop-blur-sm border border-white/30">
          ...
          {/* Properties List */}
          {loading ? null : filteredProperties.length === 0 ? (
            <div className="text-center py-12">
              ...
              <Link to="/list-property"><Button>Add Your First Property</Button></Link>
            </div>
          ) : (
            <>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center text-gray-600 mb-2">
                            ...
                            <span className="text-sm">{property.street}, {property.city} {property.postcode}</span>
                          </div>
                          <div className="flex items-center gap-4 text-sm text-gray-600 mb-3">
                            <div className="flex items-center"><Bed className="w-4 h-4 mr-1" />{property.bedrooms} bed</div>
                            <div className="flex items-center"><Bath className="w-4 h-4 mr-1" />{property.bathrooms} bath</div>
                            <div className="flex items-center"><DollarSign className="w-4 h-4 mr-1" />£{property.price.toLocaleString()}</div>
                            <div className="flex items-center"><Calendar className="w-4 h-4 mr-1" />{new Date(property.created_at).toLocaleDateString()}</div>
                          </div>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg p-4">
                  <div className="text-sm text-gray-600">
                    Showing {startIndex + 1}–{Math.min(startIndex + propertiesPerPage, filteredProperties.length)} of {filteredProperties.length}
                  </div>
                  <div className="flex items-center gap-2">
