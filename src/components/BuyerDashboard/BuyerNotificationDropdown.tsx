// src/components/BuyerDashboard/BuyerNotificationDropdown.tsx
...
interface BuyerNotificationDropdownProps {
  onShowAll: () => void;
}

export const BuyerNotificationDropdown = ({ onShowAll }: BuyerNotificationDropdownProps) => {
  ...
  const getNotificationIcon = (type: Notification['type']) => {
    switch (type) {
      ...
      case 'resolved': return <CheckCircle className="w-5 h-5 text-emerald-600" />;
            ...
            {loading ? (
              ...
              <div className="divide-y divide-gray-100 dark:divide-gray-800">
                {notifications.map((notification) => (
                  <div
                    key={notification.id}
                    className={`p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors ${!notification.read ? 'bg-emerald-50/50 dark:bg-emerald-900/10' : ''}`}
